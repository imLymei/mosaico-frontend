...
async function addPasswordToFile(blob, password, encryptPassword) {
  ...
  console.log("DEFAULT BYTES", pixelsBytes);
  ...
  console.log("Hiding password starting from: " + passwordByteOffset);
  ...
  console.log(
    "Total Decoy END_PIXEL: " +
      (pixelsBytes.filter((byte) => byte === END_PIXEL).length - 1),
  );

  console.log("EDITED BYTES SAVING", pixelsBytes);
 ...
 */
async function getPasswordFromFile(blob, encryptPassword) {
  const bitmap = await createImageBitmap(blob);
  ...
  console.log("EDITED BYTES ON RECOVERY", pixelsBytes);

  let byteIndex = calculateByteOffset(encryptPassword, pixelsBytes.length);

  console.log("Searching password starting from: " + byteIndex);
  ...
  console.log("ProbablyPassword", probablyPassword.join(""));
