...
function uploadItemPrompt() {
  ...
  function clearUploadErrors() {
    const ids = ["upload-error", "upload-name-error", "upload-secret-error", "upload-password-error", "upload-file-error"];
    for (const id of ids) {
      /** @type {HTMLParagraphElement | null} */
  ...
  function validateName() {
    ...
    if (!val || !val.trim()) {
      ...
      showFieldError("upload-name-error", "Name must be at least 3 characters.");
    } else if (val.length > 100) {
      showFieldError("upload-name-error", "Name must be 100 characters or fewer.");
    } else {
      /** @type {HTMLParagraphElement | null} */
      const el = document.getElementById("upload-name-error");
      if (el) { el.hidden = true; el.textContent = ""; }
  ...
  function validateSecret() {
    ...
    if (!val || !val.trim()) {
      ...
      showFieldError("upload-secret-error", "Secret must be at least 3 characters.");
    } else if (val.length > 500) {
      showFieldError("upload-secret-error", "Secret must be 500 characters or fewer.");
    } else {
      /** @type {HTMLParagraphElement | null} */
  ...
  function validatePassword() {
    ...
    if (!val || !val.trim()) {
      ...
      showFieldError("upload-password-error", "Key must be at least 3 characters.");
    } else if (val.length > 100) {
      showFieldError("upload-password-error", "Key must be 100 characters or fewer.");
    } else {
      /** @type {HTMLParagraphElement | null} */
  ...
  /** @type {HTMLInputElement | null} */
  const nameInput = document.getElementById("upload-name-input");
  nameInput?.addEventListener("input", () => { validateName(); updateUploadButton(); });

  /** @type {HTMLInputElement | null} */
  const secretInput = document.getElementById("upload-secret-input");
  secretInput?.addEventListener("input", () => { validateSecret(); updateUploadButton(); });

  /** @type {HTMLInputElement | null} */
  const passwordInput = document.getElementById("upload-password-input");
  passwordInput?.addEventListener("input", () => { validatePassword(); updateUploadButton(); });
  ...
  confirmBtn?.addEventListener("click", () => {
    ...
    if (secretError) { secretError.hidden = true; secretError.textContent = ""; }
