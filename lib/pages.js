...
function uploadItemPrompt() {
  ...
  /** @type {HTMLInputElement | null} */
  const secretInput = document.getElementById("upload-secret-input");
  secretInput?.addEventListener("input", updateUploadButton);

  /** @type {HTMLInputElement | null} */
  const passwordInput = document.getElementById("upload-password-input");
  passwordInput?.addEventListener("input", updateUploadButton);
  ...
  confirmBtn?.addEventListener("click", () => {
    ...
    if (!secret) {
      ...
      if (errorP) {
        errorP.textContent = "Secret to embed is required.";
        errorP.hidden = false;
      }
      return;
    }
    ...
    if (!encryptionKey) {
      ...
      const errorP = document.getElementById("upload-error");
      if (errorP) {
        errorP.textContent = "Encryption key is required.";
        errorP.hidden = false;
      }
      return;
    }

    uploadItem(fileInput.files[0], customName, secret, encryptionKey);
    overlay.remove();
  });
}

async function uploadItem(file, customName, secret, encryptionKey) {
  if (!currentVaultId) return;
